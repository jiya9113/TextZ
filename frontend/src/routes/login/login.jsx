...
import { loginUser, validUser } from "../../api/auth";
...
const Login = () => {
    ...
    const formSubmit = async (event) => {
        ...
        if(email.includes("@") && password.length > 6){
            setIsLoading(true);
            const response = await loginUser(formField);
            if(response){
                const { data } = response;
                if(data?.token){
            ...
            <div className='bg-[#121418] w-[100vw] h-[100vh] flex justify-center items-center'>
                <div className='w-[90%] sm:w-[400px] pl-0 ml-0 h-[400px] sm:pl-0 sm:ml-9 mt-20 relative'>
                    ...
                    <form className='flex flex-col gap-y-3 mt-[12%]' onSubmit={formSubmit}>
                        <div>
                            <input className="w-[100%] sm:w-[80%] bg-[#222222] h-[50px] pl-3 text-[#ffff]" onChange={onChangeHandler} name="email" type="text" placeholder='Email' value={email} required />
                        </div>
                        <div className='relative'>
                            <input className='w-[100%] sm:w-[80%] bg-[#222222] h-[50px] pl-3 text-[#ffff]' onChange={onChangeHandler} type={showPass ? "text" : "password"} name="password" placeholder='Password' value={password} required />
                            {
