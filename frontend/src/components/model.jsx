...
const Model = (props) => {
  ...
  useEffect(() => {
    setMembers(activeChat?.users.map((user) => user));
  }, [activeChat]);
          ...
          >
            <Box sx={style}>
              ...
              <div>
                ...
                <div>
                  <form className='mt-5 flex flex-col gap-y-3' onSubmit={(event) => event.preventDefault()}>
                    <input onChange={(event) => setName(event.target.value)} value={name} className="border-[#c4ccd5] border-[1px] text-[13.5px] py-[4px] px-2 w-[100%]" type="text" name="chatName" placeholder="Group Name" required />
                    <input onChange={(event) => setSearch(event.target.value)} className="border-[#c4ccd5] border-[1px] text-[13.5px] py-[4px] px-2 w-[100%]" type="text" name="users" placeholder="add users" />
                  </form>
